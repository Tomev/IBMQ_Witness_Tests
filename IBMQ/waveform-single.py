...
def run_scripts():
    ...
    for i in range(len(TOKENS)):
        try:
            ...
            print(alert)
            print(i,TOKEN_VARIABLES[i])
        print(i)
        #continue
        extr=DirectionExtractor('ibm_brisbane',token=TOKENS[TOKEN_VARIABLES[i]])
        a,b=extr.extract_directions()
        for qubit in [0,29,37,67,75,105,113]:
            qubits=[j for j in range(qubit,qubit+4)]
            h= len(qubits)-1
            di=[]
            for j in range(h):
