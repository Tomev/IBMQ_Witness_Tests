...
def main():
   ...
   # inequality_values_mean = []
    ...
    for qubit_set_idx in range(n_qubit_sets):
        ...
        ebaC=4-(c[0]**2 + c[3]**2 + c[2]**2 + c[1]**2)/(n_shots)**2
        print(baC,sqrt(ebaC/n_shots)/4)
        print("abC")
        abC= (c[4] + c[7] + c[6] + c[5]) / (
            n_shots * 4
        )
        eabC=4-(c[4]**2 + c[7]**2 + c[6]**2 + c[5]**2)/(n_shots)**2
    ...
    df = pd.DataFrame(
        columns=["qubits","LGBA", "LGeBA","LGAB", "LGeAB", "LGBaC","LGeBaC","LGaBC","LGeaBC", "LGbAC","LGebAC",\
                 "LGAbC", "LGeAbC","order","eorder","ABC","eABC","BAC","eBAC",\
                 "AbC","eAbC","bAC","ebAC","AbC","eAbC","BaC","eBaC","aBC","eaBC",\
    ...
    for i in indices:
        ...
        df.loc[i, "LGBaC"] = inequality_values_BaC[i]
        df.loc[i, "LGeBaC"] = inequality_errors_BaC[i]
        df.loc[i, "LGaBC"] = inequality_values_aBC[i]
        df.loc[i, "LGeaBC"] = inequality_errors_aBC[i]
        df.loc[i, "LGbAC"] = inequality_values_bAC[i]
        df.loc[i, "LGebAC"] = inequality_errors_bAC[i]
        df.loc[i, "LGAbC"] = inequality_values_AbC[i]
        df.loc[i, "LGeAbC"] = inequality_errors_AbC[i]
        df.loc[i, "order"] = order[i]
        df.loc[i, "eorder"] = eorder[i]
    ...
    df.to_csv(f"lgy/{results_path}/{results_path}.csv")
