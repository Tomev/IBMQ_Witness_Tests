...
def main():
    ...
    for i in range(n_jobs):  # metadata["jobs"]):
        pd_result = pd.read_csv(
            "lga/"+ results_path + "/results_tests_" + str(i) + ".csv", index_col=0
        )
   ...
   # inequality_values_mean = []
    ...
    for qubit_set_idx in range(n_qubit_sets):
        ...
        inequality_values_BA.append(BAC + (bAC+BaC)*sqrt(2) + baC*2)
        inequality_values_AB.append(ABC + (AbC+aBC)*sqrt(2) + abC*2)
        inequality_errors_BA.append(sqrt((eBAC + (ebAC+eBaC)* weak_meas_rotation_angle*2 + ebaC* weak_meas_rotation_angle* weak_meas_rotation_angle*4)/n_shots)/(4 * weak_meas_rotation_angle * weak_meas_rotation_angle))
        inequality_errors_AB.append(sqrt((eABC + (eAbC+eaBC)* weak_meas_rotation_angle*2 + eabC* weak_meas_rotation_angle* weak_meas_rotation_angle*4)/n_shots)/(4 * weak_meas_rotation_angle * weak_meas_rotation_angle))
        val_abC.append(abC)
        val_baC.append(baC)
    ...
    df.to_csv(f"lga/{results_path}/{results_path}.csv")
